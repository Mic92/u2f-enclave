...
//!
//! Reproduce what the SNP firmware hashes over the launched pages so a
//! relying party can derive the expected measurement from the ELF alone,
//! on any machine. Per firmware ABI §8.17.2; VMSA template per
//! `arch/x86/kvm/svm/sev.c::sev_es_sync_vmsa` and verified byte-for-byte
//! against `print_hex_dump_debug` output on a 6.18 kernel. Every input is
//! fixed by this binary, so the digest is too.
